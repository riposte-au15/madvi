...
class SearchMadrigalPapers(Component):
    display_name = "Search Madrigal Papers"
    description = "Tool: searches synthesized Madrigal papers table (Q1–Q7) in Chroma."
    icon = "search"
    ...
    def run(self) -> Message:
        try:
            ...
            if not api_key:
                return Message(text="ERROR(search_madrigal_papers): Missing OpenAI API key")
            ...
            for i, doc in enumerate(docs):
                ...
                title = meta.get("title") or meta.get("paper") or meta.get("source") or ""
                year = meta.get("year") or ""
                snippet = doc[: self.MAX_DOC_CHARS].strip()

                header = f"[{i+1}] {title} {year}".strip() if title or year else f"[{i+1}]"
                piece = f"{header}\n{snippet}"
            ...
            return Message(text=f"ERROR(search_madrigal_papers): {type(e).__name__}: {e}")
